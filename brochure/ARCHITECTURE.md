...
This is one application project in the single Angular workspace rooted at `shared/web`. It does not have its own `angular.json` or `package.json` — both live in `shared/web`, and `brochure` is registered there as the `brochure` project (`shared/web/angular.json`). Its `tsconfig.json` extends `shared/web/tsconfig.json` and maps the shared libs:

- `@start9labs/marketplace` → `shared/web/marketplace`
- `@start9labs/shared` → `shared/web/shared`
...
Taiga UI 5 + the shared marketplace/shared libraries. The app is dark-themed (`tuiTheme="dark"`, `provideTaiga({ mode: 'dark' })`). User-facing strings go through the `i18n` pipe (`I18N_PROVIDERS`), with dictionaries living in `shared/web/shared/src/i18n`.
