...
- Build/lint/serve config: `shared-libs/ts-modules/angular.json` (project `brochure`) — **not** here.
- npm scripts: `shared-libs/ts-modules/package.json`. Run them from `shared-libs/ts-modules`.
- `brochure/tsconfig.json` extends `shared-libs/ts-modules/tsconfig.json` and only adds path mappings; the real compiler settings live there.
...
## Build / run / check (always from `shared-libs/ts-modules`)
