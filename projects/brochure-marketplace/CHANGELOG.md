...
- Migrated into the start-technologies monorepo as an application project of the `shared-libs/ts-modules` Angular workspace. Build and serve via the `brochure` Angular project (`npm run build:brochure` / `npm run start:brochure` from `shared-libs/ts-modules`); deploys automatically to marketplace.start9.com on merge to `master`.
