...
- Top level also holds the shared build infra (`build/`, `Makefile`), `apt/`, the shared `debian/dpkg-build.sh`, repo-level `scripts/`, `rfcs/` (protocol drafts), and `vendor/patch-db/` (git submodule, consumed by `start-core` and web).
