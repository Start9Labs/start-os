...
- **Init progress reporting via `FullProgressTracker`.** Service code no longer calls a progress effect directly. The init harness builds a root `FullProgressTracker` (wired to the host `setInitProgress` effect) and hands each init handler its own nested tracker as a third argument: `setupOnInit(async (effects, kind, progress) => …)`. Add phases to `progress` and call `await progress.sync(effects)` to surface progress in the "Installing" / "Updating" phase of the install — `sync()` walks up to the root and calls the effect under the hood. Multiple inits each add their own phases unaware of one another; even inits that report nothing advance the bar as each completes. Migrations receive the same tracker via `migrations.up`/`down`/`other` opts (`async ({ effects, progress }) => …`), so a long data migration can show progress during an update. `FullProgressTracker` is exported from `utils` (`utils.FullProgressTracker`) — no deep import needed. The underlying `effects.setInitProgress` / `effects.setBackupProgress` remain but are now internal (use `tracker.sync()`)
- **Backup/restore progress mirrors the same structure.** `Backups.createBackup` and `restoreBackup` build a `FullProgressTracker` and report via `tracker.sync(effects)` instead of calling the effect directly. The pre/post backup and restore hooks (`setPreBackup`, `setPostBackup`, `setPreRestore`, `setPostRestore`) now receive a `FullProgressTracker` as a second argument so custom work (DB dumps, etc.) can report sub-progress; restore progress flows through the init tracker since restore runs during init. Existing hooks that ignore the new argument keep working
...
- **TypeScript bumped `^5.9.3` → `^6.0.3`, and the build migrated off the deprecated `moduleResolution: "node"` (node10).** TS 6.0 deprecates node10 resolution (removed entirely in TS 7.0). The SDK's own `base` / `package` tsconfigs now use `moduleResolution: "bundler"` (keeping `module: "commonjs"` — emitted CJS is byte-identical and the published `.d.ts` API surface is unchanged) plus an explicit `rootDir` (TS 6.0 changed the default to the tsconfig directory). The shipped `tsconfig.base.json` that packages `extends` now uses `moduleResolution: "bundler"` + `module: "preserve"`, putting the fleet on modern resolution before TS 7. No SDK source changes were required
