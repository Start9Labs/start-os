...
- `effects.setInitProgress({ progress })` reports progress from a service's init procedure, mirroring `effects.setBackupProgress`. The host routes the value into the package's install/update finalization phase, so it surfaces in the "Installing" / "Updating" phase of the install progress (same `FullProgress` wire format as backups — a leaf `Progress`, or a nested `FullProgress` built with the `FullProgressTracker` utility). `setupInit` drives this automatically: it reports one step per composed init handler as each completes, so multi-step inits show progress without any extra code. No-op outside the install/update transition
