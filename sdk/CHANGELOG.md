...
- `userspaceFilesystems` and `virtualNetworking` manifest flags split the former `nestedRuntime` flag into its two independent grants. `userspaceFilesystems` mounts `/dev/fuse` and grants the cgroup device permission for fuse-overlayfs storage (the rootless driver behind a nested OCI runtime). `virtualNetworking` mounts `/dev/net/tun` **and grants CAP_NET_ADMIN** (scoped to the container's user namespace) so a service can bring up kernel tun interfaces for VPN / WireGuard / tun-class workloads — a network privilege `nestedRuntime` never granted despite its name
