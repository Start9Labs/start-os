...
- Actions can be marked **public** via the metadata passed to `Action.withInput` / `Action.withoutInput` (`public: true`; the field is optional and defaults to `false`). A public action may be invoked **directly** by a dependent service through `effects.action.run({ packageId, actionId, input })`, instead of only being requested via `effects.action.createTask(...)`. Non-public actions remain callable only by their owning package (and the user); a direct cross-package `run` against a non-public action is rejected. Direct runs still go through the owning service and honor the action's `visibility` (disabled) and `allowedStatuses` checks
