  ...
  build(isReady: AnyNeverCond<[Manifest], 'Build not ready', true>) {
    ...
    return {
      ...
      notification: {
         ...
         *
         *   The shape of `data` is determined by `code`:
         *   - `code: 0` — a plain notification with no extra payload (omit `data`).
         *     The list-row shows `title` and `message` only.
         *   - `code: 2` — a notification with a long-form, markdown-compatible
         *     `data` body. The list-row shows `title` and `message`; clicking
         *     "View Details" opens `data` rendered as markdown in a large modal.
         *     Use this for release notes, post-update changelogs, structured
         *     error reports, or anything that needs more space than `message`.
         *     `data` should be markdown text, not a short status string.
         *
         *   Code `1` is reserved for backup-report notifications generated by
         *   StartOS and cannot be created by packages.
         * @example
         * ```
